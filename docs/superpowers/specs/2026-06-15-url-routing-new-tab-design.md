...
- **Tidak refactor in-screen tab pattern yang belum ada.** Stok screen multi-tab (Katalog/Stok per Gudang/Bulk Upload/Stok Tipis) ada di spec `2026-06-14-product-photo-search-design.md` Round 5; spec ini hanya menyediakan **mekanisme URL routing**-nya, tidak ikut bikin tab UI.
...
?screen=pembelian&bnl-new-for-order=ord_xxx       # existing pattern preserved
...
`activePage` state pernah di-set dari banyak tempat (subscribe events, modal close handlers, deep-link restore, permission fallback). Refactor harus catch semua call site supaya URL state tetap konsisten. **Verification**: setelah refactor, `grep "setActivePage"` di `src/` harus hanya menemukan call yang sudah diganti `navigate(...)` atau dihapus dengan alasan. Jangan ada `setActivePage` yang lolos — itu jejak state-lokal yang akan inconsistent dengan URL.
