...
| `20260626000022` | `approve_reject_tempo_write_off_rpcs.sql` | (a) `approve_tempo_write_off(p_approval_id BIGINT) RETURNS VOID`. Verify caller is Aktif Owner via `auth.uid()`-bound lookup (same shape as fixed `verify_owner_pin`); lock approval row + verify `status='pending'` (else `APPROVAL_NOT_PENDING`); fetch satellite + order; if order no longer `INVOICE_TEMPO` (race), auto-reject approval with system reason `'race: order status changed'` and raise `ORDER_NO_LONGER_TEMPO`; UPDATE `orders` SET `status='INVOICE_WRITTEN_OFF'`, `written_off_at=now()`, `written_off_by=v_caller_admin_id`, `write_off_reason=v_satellite.reason`; call `_transition_approval(approval_id, 'approved', v_caller_admin_id, 'piutang_write_off_approve')`; INSERT audit_log `tempo_write_off_approved` with payload `{approval_id, order_id, customer_id, amount, reason}`. (b) `reject_tempo_write_off(p_approval_id BIGINT, p_reason TEXT) RETURNS VOID`. Same caller check; lock approval; call `_transition_approval` with status `rejected` and the reject reason; INSERT audit `tempo_write_off_rejected` with payload `{approval_id, order_id, reject_reason}`. Order untouched. |
...
| `approve` — race: order paid before approve | `ORDER_NO_LONGER_TEMPO: status=%`; approval auto-marked rejected with system reason | "Invoice sudah dibayar sebelum disetujui — pengajuan dibatalkan otomatis" |
...
The race "customer pays while write-off pending" is handled by lazy check at approve time: the `approve` RPC re-reads the order status and auto-rejects + raises if it's no longer `INVOICE_TEMPO`. We do NOT proactively reject pending write-offs from inside `markTempoInvoicePaid` (extra coupling, no real benefit — the lazy check is sufficient and the toast tells the Owner what happened).
