      ...
      <div className="bg-white rounded border border-gray-200 shadow-xl w-full max-w-md">
        ...
        <div className="px-4 py-4 space-y-3">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} className="w-full text-sm border border-gray-200 rounded px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caleo-gold" placeholder="PT Schneider Elektrik" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-gray-600 block mb-1">Nama Kontak</label>
              <input value={contactName} onChange={e => setContactName(e.target.value)} className="w-full text-sm border border-gray-200 rounded px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caleo-gold" placeholder="Budi Santoso" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-600 block mb-1">Nomor HP</label>
              <input value={phone} onChange={e => setPhone(e.target.value)} className="w-full text-sm border border-gray-200 rounded px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caleo-gold" placeholder="0812-xxxx-xxxx" />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-600 block mb-1">Term Pembayaran (hari)</label>
            <input type="number" min="0" value={termDays} onChange={e => setTermDays(e.target.value)} className="w-full text-sm border border-gray-200 rounded px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caleo-gold" placeholder="30" />
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-gray-200">
          ...
          <button onClick={handleSave} disabled={saving} className="text-sm font-semibold text-white bg-indigo-600 px-4 py-2 rounded hover:bg-indigo-700 disabled:opacity-50">
