...
export default function SkuPickerWithInlineCreate({ value, unitCostHint, onChange }: Props) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-30 left-0 right-0 mt-1 max-h-72 overflow-auto bg-white rounded border border-gray-200 shadow-lg">
          {results.map(s => (
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded border border-gray-200 shadow-xl w-full max-w-sm p-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-4 justify-end">
              <button type="button" onClick={() => setShowCreate(false)} className="text-sm px-3 py-2 rounded border border-gray-200 text-gray-600">Batal</button>
              <button type="button" onClick={handleCreate} disabled={saving || !draftName.trim()}
                className="text-sm px-3 py-2 rounded text-white font-semibold disabled:opacity-50"
