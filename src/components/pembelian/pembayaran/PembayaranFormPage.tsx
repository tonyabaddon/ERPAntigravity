                          ...
                          <tr key={`tag-${r.ref_id}`} className={`border-b border-gray-100 ${r.selected ? 'bg-indigo-50/30' : ''}`}>
                            ...
                            <td className="py-2">
                              <NumberInput disabled={!r.selected} value={r.amount}
                                ...
                                className={`w-full text-sm text-right py-1 px-2 rounded border ${overpay ? 'border-red-400 bg-red-50' : 'border-gray-200'} disabled:bg-gray-50 disabled:text-gray-400`} />
                            </td>
                          </tr>
                        );
                      })}
              ...
              <div className="grid grid-cols-4 gap-2">
                {METHODS.map(m => (
                  <button key={m} type="button" onClick={() => setPaymentMethod(m)}
                    className={`text-xs font-bold py-2 rounded border-2 ${
