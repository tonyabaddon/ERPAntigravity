  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded max-w-3xl w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between gap-4 mb-5 bg-emerald-50 border border-emerald-200 rounded p-3">
          <div className="flex items-center gap-3">
            {queryBlobUrl ? (
              <img src={queryBlobUrl} alt="query" className="w-16 h-16 rounded object-cover" />
            ) : (
        ...
        <div className="space-y-2">
          {results.map((r, i) => {
            ...
            return (
              <div key={r.sku} className={`rounded p-3 flex items-center gap-3 ${isBest ? 'bg-emerald-50/40 border border-emerald-300' : 'bg-white border border-slate-200'} ${isLast ? 'opacity-80' : ''}`}>
                <img src={r.photo_url} alt={r.name} className="w-14 h-14 rounded object-cover flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    {isBest && <span className="text-caleo-9 font-extrabold bg-emerald-600 text-white px-2 py-0.5 rounded-full uppercase">Best match</span>}
                    {lowStock && <span className="text-caleo-9 font-extrabold bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full uppercase">Tipis</span>}
                  </div>
                  <h4 className="text-sm font-extrabold text-[var(--color-caleo-primary)] mt-1">{r.name}</h4>
                  <p className="text-caleo-10 font-mono text-slate-500">{r.sku}</p>
                ...
                <div className="text-right flex-shrink-0">
                  <div className={`text-caleo-11 font-extrabold ${simColorClass(r.similarity)}`}>{Math.round(r.similarity * 100)}%</div>
        ...
        <div className="mt-5 pt-4 border-t border-slate-200 text-center">
          <button onClick={onClose} className="text-caleo-11 font-bold text-[var(--color-caleo-primary)] hover:underline inline-flex items-center gap-1">
