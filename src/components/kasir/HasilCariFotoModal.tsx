...
export default function HasilCariFotoModal({ isOpen, onClose, results, queryBlobUrl, queryFilename, onChangePhoto, onAddToCart }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-sm max-w-3xl w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between gap-4 mb-5 bg-emerald-50 border border-emerald-200 rounded-sm p-3">
          <div className="flex items-center gap-3">
            {queryBlobUrl ? (
              <img src={queryBlobUrl} alt="query" className="w-16 h-16 rounded-sm object-cover" />
            ) : (
              <div className="w-16 h-16 rounded-sm bg-slate-200" />
            )}
            <div>
        ...
        {isEmpty && (
          <div className="bg-amber-50 border border-amber-200 rounded-sm p-4 text-[12px] text-amber-900">
        ...
        <div className="space-y-2">
          {results.map((r, i) => {
            ...
            return (
              <div key={r.sku} className={`rounded-sm p-3 flex items-center gap-3 ${isBest ? 'bg-emerald-50/40 border border-emerald-300' : 'bg-white border border-slate-200'} ${isLast ? 'opacity-80' : ''}`}>
                <img src={r.photo_url} alt={r.name} className="w-14 h-14 rounded-sm object-cover flex-shrink-0" />
