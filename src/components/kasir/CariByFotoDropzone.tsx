...
export default function CariByFotoDropzone({ onFileSelected, onError }: Props) {
  ...
  return (
    <button
      ...
      onDrop={handleDrop}
      className={`w-full rounded p-5 text-left transition-colors border-2 border-dashed ${
        isDragging ? 'bg-violet-100 border-violet-500' : 'bg-violet-50/50 border-violet-300 hover:bg-violet-100'
      }`}
    >
      <div className="flex items-center gap-4">
        <div className="w-14 h-14 bg-violet-200 rounded flex items-center justify-center flex-shrink-0">
