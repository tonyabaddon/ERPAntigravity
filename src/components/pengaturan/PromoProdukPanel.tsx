...
function PromoModal({ initialRow, onClose, onSuccess, showToast }: ModalProps) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        {/* Header */}
        ...
        <div className="px-4 py-4 border-t border-slate-100 flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm font-semibold text-slate-600 bg-slate-100 rounded hover:bg-slate-200"
          >
            Batal
          </button>
          <button
    ...
    <div className="space-y-4" id="promo-produk">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <button
          ...
          className="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded hover:bg-blue-700 shrink-0"
        >
          + Tambah Promo
        </button>
        <input
          ...
          className="flex-1 min-w-40 border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
        />
        <select
          value={filter}
          onChange={(e) => setFilter(e.target.value as PromoFilter)}
          className="border border-slate-300 rounded px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
        >
      ...
      {/* Bulk action toolbar */}
      {selected.size > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 border border-blue-200 rounded text-sm">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-white text-slate-600 border border-slate-300 rounded text-xs font-semibold hover:bg-slate-50"
          >
            Batal
          </button>
        </div>
...
function RowMenu({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-6 z-10 bg-white border border-slate-200 rounded shadow-lg min-w-[110px] overflow-hidden">
          <button
            type="button"
