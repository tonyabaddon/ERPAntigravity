...
export default function ClipMonitorPanel() {
  ...
  return (
    <div className="bg-white rounded border border-[var(--color-caleo-mist)] p-6 shadow-sm">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-4 text-caleo-11 text-[var(--color-caleo-primary)]">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded p-4">
          <p className="text-caleo-10 font-extrabold uppercase tracking-widest text-blue-700">Indexing Upload</p>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-slate-50 border border-slate-200 rounded p-3 text-center">
          <p className="text-caleo-10 font-extrabold uppercase tracking-widest text-slate-500">Latency p50</p>
          <p className="text-lg font-black text-[var(--color-caleo-primary)] mt-1">{agg.latencyP50 != null ? `${agg.latencyP50} ms` : '—'}</p>
        </div>
        <div className="bg-slate-50 border border-slate-200 rounded p-3 text-center">
          <p className="text-caleo-10 font-extrabold uppercase tracking-widest text-slate-500">Latency p95</p>
          <p className="text-lg font-black text-[var(--color-caleo-primary)] mt-1">{agg.latencyP95 != null ? `${agg.latencyP95} ms` : '—'}</p>
        </div>
        <div className="bg-amber-50 border border-amber-200 rounded p-3 text-center">
          <p className="text-caleo-10 font-extrabold uppercase tracking-widest text-amber-700">Cold start hit</p>
          <p className="text-lg font-black text-amber-900 mt-1">{agg.search.coldStart + agg.index.coldStart} ×</p>
        </div>
      </div>

      <div className="bg-amber-50 border border-amber-200 rounded p-3">
        <p className="text-caleo-10 font-extrabold uppercase tracking-widest text-amber-700 mb-1">Sinyal kapan tindak lanjut</p>
        <ul className="text-caleo-11 text-amber-900 list-disc ml-5 space-y-0.5">
      ...
      {agg.lastErrorAt && (
        <p className="text-caleo-10 text-slate-500 italic mt-3">Last error: {new Date(agg.lastErrorAt).toLocaleString('id-ID')}</p>
      )}
    </div>
  );
}
