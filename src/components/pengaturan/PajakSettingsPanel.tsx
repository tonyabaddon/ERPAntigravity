...
export default function PajakSettingsPanel({ showToast }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* Mode picker */}
      <section>
        <label className="block text-caleo-11 font-bold text-slate-500 uppercase tracking-wider mb-3">Status Pajak Toko (regulasi 2026)</label>
      ...
      {settings.pajak_mode === 'FINAL_UMKM' && (
        <section className="border border-slate-200 rounded p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Jenis Badan Usaha</label>
            ...
            <div>
              <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Terdaftar UMKM Sejak</label>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div>
              <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Tarif PPh Final</label>
      ...
      {/* NPWP / NIK */}
      <section className="border border-slate-200 rounded p-5">
        <h3 className="font-bold text-sm text-[var(--color-caleo-primary)] mb-3">🆔 NPWP / NIK <span className="text-caleo-10 text-slate-500 italic">(Regulasi DJP Juli 2024)</span></h3>
        <div className="space-y-2">
          <label className="flex items-center gap-3 p-3 border border-slate-200 rounded cursor-pointer hover:bg-slate-50">
            ...
            <div className="flex-1">
              ...
              <div className="text-caleo-11 text-slate-500">Format 16 digit.</div>
            </div>
          </label>
          <label className="flex items-center gap-3 p-3 border border-slate-200 rounded cursor-pointer hover:bg-slate-50">
            ...
            <div className="flex-1">
              ...
              <div className="text-caleo-11 text-slate-500">Untuk PT/CV/Koperasi.</div>
            </div>
          </label>
        </div>
        <div className="mt-3">
          <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">
      ...
      {settings.pajak_mode === 'PKP' && (
        <section className="border border-slate-200 rounded p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Terdaftar PKP Sejak</label>
            ...
            <div></div>
            <div>
              <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Tarif PPN Umum</label>
              <div className="flex items-center gap-2">
            ...
            <div>
              <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Tarif PPN Barang Mewah</label>
              <div className="flex items-center gap-2">
          ...
          <label className="flex items-start gap-2 text-xs text-slate-700 mt-4">
            ...
            <span><strong>Aktifkan e-Faktur 3.0</strong><br />
              <span className="text-caleo-11 text-slate-500">Generate XML e-Faktur. <em>Phase 1 placeholder; integrasi DJP defer V2.</em></span></span>
          </label>
          <div className="mt-3">
            <label className="block text-caleo-11 font-bold text-slate-500 uppercase mb-1">Coretax ID</label>
            <input type="text" value={settings.pajak_coretax_id ?? ''}
                   onChange={e => save({ pajak_coretax_id: e.target.value })}
                   className="w-full px-3 py-2 text-sm border border-slate-300 rounded" />
            <div className="text-caleo-11 text-slate-400 mt-1"><em>Phase 1 storage saja; real-time push defer V2.</em></div>
          </div>
        </section>
      )}

      {/* Regulation footer */}
      <section className="bg-slate-50 rounded p-4 text-caleo-11 text-slate-500">
