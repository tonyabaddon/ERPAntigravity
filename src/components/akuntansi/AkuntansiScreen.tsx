...
export default function AkuntansiScreen({ currentUser, showToast }: Props) {
  ...
  if (loading) {
    return (
      <div className="p-8 text-[#43474e] text-caleo-13">Memuat...</div>
    );
  }
    ...
    if (!isOwner) {
      return (
        <div className="p-8 max-w-2xl">
          ...
          <div className="border border-[var(--color-caleo-mist-dark)] bg-[#fafbff] rounded p-6 text-caleo-13 text-[#43474e]">
        ...
        {tabs.map(t => {
          ...
          return (
            <button
              ...
              className={`px-4 py-3 text-caleo-13 whitespace-nowrap transition-colors inline-flex items-center gap-2 ${
                activeTab === t.key
