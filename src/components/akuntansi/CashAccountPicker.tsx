...
export default function CashAccountPicker({
      ...
      <div>
        {label && (
          <label className="block font-bold mb-1 text-caleo-13" style={{ color: '#1e3d60' }}>
            {label}
            {required && <span className="text-caleo-danger"> *</span>}
          </label>
        )}
        <select
          disabled
  ...
  if (error) {
    return (
      <div>
        ...
        <select
          disabled
          className="w-full border border-rose-300 rounded px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caleo-gold focus-visible:ring-offset-2 bg-white disabled:opacity-60"
        >
  ...
  return (
    <div>
      ...
      {required && !value && (
        <p className="text-caleo-10 text-caleo-danger mt-1">Wajib dipilih</p>
      )}
    </div>
  );
}
