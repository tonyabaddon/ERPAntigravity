...
export default function CompletionSummary({ orderPct, mutasiPct, cashPct }: Props) {
  ...
  return (
    <div className="bg-white/85 backdrop-blur-xl rounded-[1.5rem] p-5 border border-[var(--color-caleo-mist)] shadow-sm flex items-center justify-between">
      <div>
        <div className="text-caleo-11 font-black uppercase tracking-widest text-[var(--color-caleo-primary)]">🎯 Target Final · Semua Punya Pasangan</div>
        <div className="text-caleo-10 text-slate-500 font-semibold mt-0.5">Tutup buku diizinkan setelah ketiga kolom 100% atau reason untuk yang tidak match</div>
      </div>
      <div className="flex gap-6 items-center">
        {([['Order', orderPct], ['Mutasi', mutasiPct], ['Kas', cashPct]] as const).map(([label, pct]) => (
          <div key={label} className="text-center">
            <div className="text-caleo-10 font-bold uppercase text-slate-500">{label}</div>
