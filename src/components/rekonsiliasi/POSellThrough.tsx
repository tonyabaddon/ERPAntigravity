  ...
  return (
    <div className="bg-white/78 backdrop-blur-xl rounded-[1.75rem] border border-[var(--color-caleo-mist)] shadow-sm overflow-hidden">
      <div className="px-4 py-4 border-b border-[var(--color-caleo-mist)]">
        <div className="text-caleo-11 font-black uppercase tracking-widest text-[var(--color-caleo-primary)]">📦 Pembelian dari Supplier Bulan Ini</div>
        <div className="text-caleo-10 text-slate-500 font-semibold mt-0.5">Klik ▶ untuk lihat per-barang & sales order yang membeli</div>
      </div>
            ...
            <div key={po.id} className="rounded border overflow-hidden" style={{ background: bgColor, borderColor }}>
              <div onClick={toggle} className="p-4 flex justify-between items-start cursor-pointer">
                <div>
                  ...
                  <div className="text-caleo-10 text-slate-500 font-semibold mt-0.5">Terima {fmtDate(po.received_at)} · Tempo {po.payment_due_at ? fmtDate(po.payment_due_at) : '—'}</div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right"><div className="text-caleo-10 font-bold text-slate-500">Bayar</div><div className="text-sm font-black text-[var(--color-caleo-primary)]">{fmt(po.total)}</div></div>
                  <div style={{ width: 140 }} className="text-right">
                    <div className={`text-caleo-10 font-bold ${labelColor}`}>Laku {pct}%</div>
                    ...
                    <div key={it.sku}>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {it.consumed_by.map((c, idx) => (
                          <span key={idx} className="text-caleo-9 font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full font-mono">#{c.order_id.slice(0, 6)} · {c.qty} · {fmtDate(c.date)}</span>
                        ))}
                        {it.consumed_by.length === 0 && <span className="text-caleo-10 text-slate-400 font-semibold">— belum ada penjualan —</span>}
                      </div>
                    </div>
                  ))}
                </div>
