  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(1,39,73,0.4)' }} onClick={onCancel}>
      <div className="bg-white rounded-sm p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <select value={form.bank_code} onChange={e => setForm({ ...form, bank_code: e.target.value as BankAccount['bank_code'] })} className="w-full mb-3 px-3 py-2 border border-[#e5eeff] rounded-sm text-xs">
        ...
        <input value={form.account_number} onChange={e => setForm({ ...form, account_number: e.target.value })} className="w-full mb-3 px-3 py-2 border border-[#e5eeff] rounded-sm text-xs" />
        <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Label</label>
        <input value={form.account_label} onChange={e => setForm({ ...form, account_label: e.target.value })} className="w-full mb-3 px-3 py-2 border border-[#e5eeff] rounded-sm text-xs" placeholder="BCA Bisnis Operasional 8420" />
        <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Tujuan</label>
        <select value={form.purpose} onChange={e => setForm({ ...form, purpose: e.target.value as BankAccount['purpose'] })} className="w-full mb-4 px-3 py-2 border border-[#e5eeff] rounded-sm text-xs">
