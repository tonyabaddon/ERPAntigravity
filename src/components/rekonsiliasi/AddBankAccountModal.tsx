  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(1,39,73,0.4)' }} onClick={onCancel}>
      <div className="bg-white rounded-3xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <select value={form.bank_code} onChange={e => setForm({ ...form, bank_code: e.target.value as BankAccount['bank_code'] })} className="w-full mb-3 px-3 py-2 border border-[#e5eeff] rounded-xl text-xs">
        ...
        <select value={form.purpose} onChange={e => setForm({ ...form, purpose: e.target.value as BankAccount['purpose'] })} className="w-full mb-4 px-3 py-2 border border-[#e5eeff] rounded-xl text-xs">
