...
export function WhatsappStrip({ phone, chatUrl, onPhoneChange, onChatUrlChange }: WhatsappStripProps) {
  return (
    <div className="bg-gradient-to-r from-green-100 to-green-50 border border-green-300 border-l-4 border-l-green-600 rounded px-4 py-3 mb-4 flex items-start gap-3">
      ...
      <div className="flex-1">
        <label className="text-caleo-11 font-extrabold text-green-700 uppercase tracking-widest block">
          Catat Pesanan WhatsApp Manual
        </label>
        <p className="text-caleo-11 text-green-800 mt-0.5 mb-2">Pesanan WA yang di-input manual oleh admin.</p>
        <div className="grid grid-cols-[1fr_1.4fr] gap-2">
          <input
            value={phone}
            onChange={e => onPhoneChange(e.target.value)}
            placeholder="No. WA pelanggan"
            className="bg-white border border-green-300 rounded px-3 py-2 text-caleo-13 font-bold text-green-900"
          />
          <input
            value={chatUrl}
