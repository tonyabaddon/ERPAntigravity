...
export default function RakitInlineForm({ type, serviceTypeName, onAdd, onCancel }: RakitInlineFormProps) {
  ...
  return (
    <div className={`bg-white border ${isCustom ? 'border-sky-300' : 'border-orange-300'} rounded p-3 space-y-3`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-caleo-10 font-extrabold uppercase tracking-wider ${
      ...
      <div>
        ...
        <input
          type="text"
          className="w-full bg-slate-50 border border-slate-200 rounded px-3 py-2 text-caleo-13 focus-visible:outline-none focus:border-emerald-500"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <div className="text-caleo-10 font-extrabold text-slate-500 uppercase tracking-widest mb-1.5">Estimasi Harga (quote disepakati)</div>
          <input
            type="number"
            min={0}
            className="w-full bg-slate-50 border border-slate-200 rounded px-3 py-2 text-caleo-13 focus-visible:outline-none focus:border-emerald-500"
        ...
        <div>
          <div className="text-caleo-10 font-extrabold text-slate-500 uppercase tracking-widest mb-1.5">HPP (modal)</div>
          <input
            type="number"
            min={0}
            className="w-full bg-slate-50 border border-slate-200 rounded px-3 py-2 text-caleo-13 focus-visible:outline-none focus:border-emerald-500"
      ...
      <div className="flex justify-end gap-2 pt-1">
        <button type="button" onClick={onCancel} className="px-3 py-2 rounded text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50">
          Batal
        </button>
        <button
          type="button"
          onClick={submit}
          disabled={!canSubmit}
          className={`px-3 py-2 rounded text-xs font-extrabold text-white transition ${
