...
function CartRow({
  ...
  return (
    <div
      key={item._key}
      className={`p-3 bg-slate-50 border border-slate-200 rounded-sm mb-2 items-start text-[12px] ${
        modulDiskonOn
    ...
    >
      ...
      {/* Warehouse selector */}
      <div className="flex gap-0.5 bg-white border border-slate-200 rounded-sm p-0.5">
        <WarehousePicker
          mode="single"
...
export default function CartRows({ items, stocks, onQtyChange, onWarehouseChange, onRemove, onDiscountChange, rakitLines, onRemoveRakit, stockByWarehouseSku, serviceTypes, modulDiskonOn = true, activeTier, showTierPill, promos, stockQtyTiers, onToggleManual, onManualPriceOverride }: CartRowsProps) {
  ...
  if (totalLineCount === 0) {
    return (
      <div className="px-6 py-8 text-center text-slate-400 text-[13px] bg-slate-50 border border-dashed border-slate-300 rounded-sm">
  ...
  return (
    <>
      <div className="bg-emerald-50 border border-emerald-300 rounded-sm px-3 py-2 mb-2 flex justify-between items-center">
        ...
        <>
          ...
          {rakitLines.map(r => {
            ...
            return (
              <div
                key={r.id}
                className="rounded-sm p-3 mb-2 grid grid-cols-[1fr_auto_auto] gap-3 items-center text-[12px]"
                style={{
                  background: isCustom
