...
const LEAD_BADGE: Record<string, { label: string; className: string }> = {
  ...
  ORDERED:     { label: 'Selesai',  className: 'bg-emerald-100 text-caleo-success' },
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded bg-[var(--color-caleo-primary)] px-4 py-2 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
        >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded border border-gray-200 shadow-xl w-full max-w-lg mx-4">
      ...
      <div className="flex bg-white rounded border border-gray-200 overflow-hidden" style={{ minHeight: '520px' }}>
          ...
          {!selectedId ? (
            ...
            <>
              ...
              <div className="bg-[var(--color-caleo-primary)] text-white p-4 flex items-center gap-3 shrink-0">
                <div className="w-11 h-11 rounded bg-[#2d8a4e] flex items-center justify-center text-lg font-extrabold shrink-0">
                ...
                {editing ? (
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      disabled={saving}
                      className="flex items-center gap-1 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white text-xs font-bold px-2.5 py-1.5 rounded transition-colors"
                    >
                      <Check className="w-3.5 h-3.5" />{saving ? '...' : 'Simpan'}
                    </button>
                    <button
                      onClick={() => setEditing(false)}
                      className="flex items-center gap-1 bg-white/10 hover:bg-white/20 text-white text-xs px-2.5 py-1.5 rounded transition-colors"
                    >
              ...
              <div className="px-4 py-4">
                ...
                {salesEntries.length === 0 ? (
                  ...
                  salesEntries.map(entry => {
                    ...
                    const totalColor = TOTAL_COLOR[entry.status] ?? (entry.status === 'PAID' ? 'text-caleo-success' : 'text-gray-700');
                    return (
                      <div key={entry.id} className="border border-gray-200 rounded p-3 mb-2 last:mb-0 text-xs">
