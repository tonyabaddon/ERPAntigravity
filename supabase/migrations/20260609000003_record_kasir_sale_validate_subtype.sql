-- Add p_payment_subtype + p_dp_input_type validation to record_kasir_sale.
--
-- Pre-existing behavior: invalid values for these inputs would propagate
-- silently through the FIFO + counter + customer logic and only fail at
-- the kasir_transactions CHECK constraint with an opaque Postgres error
-- ("new row violates check constraint"). Atomicity was still correct
-- (full rollback), but the cashier or caller had no idea what was wrong.
--
-- After this change: invalid values raise a clear RAISE EXCEPTION at the
-- top of the function — before stock mutations, before the counter bump,
-- before customer lookup. Same fail-fast pattern as the existing channel
-- and payment_method checks.
--
-- payment_subtype whitelist comes from chk_kasir_payment_subtype:
--   NULL | 'debit' | 'qris'
-- dp_input_type whitelist comes from chk_kasir_dp_input_type:
--   NULL | 'AMOUNT' | 'PERCENT'
--
-- The function body is otherwise identical to 20260609000001.
