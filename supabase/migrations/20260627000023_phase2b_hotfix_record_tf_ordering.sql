-- supabase/migrations/20260627000023_phase2b_hotfix_record_tf_ordering.sql
-- Phase 2b hotfix #4: record_tukar_faktur insert ordering broke after hotfix #3
-- added the FK purchase_invoices.tukar_faktur_id → tukar_faktur(id).
-- Original ordering: insert quick_add Tagihans first (referencing v_tf_id) → insert TF.
-- After FK, this fails with 23503 because v_tf_id doesn't exist yet in tukar_faktur.
-- Fix: insert TF first with placeholder total=0, then insert quick-add Tagihans
-- with valid FK, then UPDATE TF total_amount.
...
BEGIN
  ...
  -- ORDER FIX: Insert TF FIRST (with placeholder total=0) so FK constraint can resolve.
  INSERT INTO public.tukar_faktur (
    ...
    0, v_photo_urls, v_notes, auth.uid()
  );

  -- Now safe to insert quick_add Tagihans (FK target exists)
  FOR v_quick_item IN SELECT * FROM jsonb_array_elements(v_quick_add) LOOP
    ...
    INSERT INTO public.purchase_invoices (
      ...
      COALESCE(v_quick_item->>'payment_method', 'TRANSFER'),
  ...
  -- Update TF with computed total_amount
  UPDATE public.tukar_faktur SET total_amount = v_total WHERE id = v_tf_id;
