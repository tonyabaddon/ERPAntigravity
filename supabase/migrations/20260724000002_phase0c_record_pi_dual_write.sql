-- 20260724000002_phase0c_record_pi_dual_write.sql
--
-- Phase 0c Task 2: Add soft-fail GL dual-write to record_pi.
--
-- GL shape (single JE per Tagihan created):
--   D  1-1510  Persediaan     total (v_subtotal)
--   K  2-1100  Hutang Usaha   total (v_subtotal)
--
-- Non-PKP only (Garindo): no PPN split. Phase 1 revisit for PKP tenants.
--
-- TODO Phase 1: PASSTHROUGH type does not update stock_lots, so debiting 1-1510
-- Persediaan is semantically incorrect for pass-through purchases. When branching
-- is added, PASSTHROUGH should debit a different account (e.g., 5-1200 HPP
-- Langsung or equivalent). For Phase 0c observability this is acceptable.
--
-- NOTE: LUNAS-at-create synthesizes a pembayaran row directly (does NOT call
-- record_pembayaran), so Phase 0b's pembayaran dual-write (D 2-1100 K cash) does
-- NOT automatically fire. Net: LUNAS Tagihan books only the AP-creation JE here
-- (D Persediaan K Hutang). The payment leg is missing until Phase 1 full-accrual.
--
-- Soft-fail: all GL errors caught → anomaly logged to gl_dual_write_anomalies
-- → RAISE WARNING → business RETURN proceeds normally.
--
-- Changes from 20260630000006:
--   1. Capture v_purchase_date before INSERT (for JE date consistency).
--   2. Add GL dual-write block before RETURN.
--   Existing business logic 100% preserved.
...
BEGIN
  ...
  -- Posts a balanced JE: D 1-1510 Persediaan / K 2-1100 Hutang Usaha.
  -- All errors are caught; record_pi always returns its business result.
