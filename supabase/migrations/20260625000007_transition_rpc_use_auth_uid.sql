-- Replace transition_order_stage to use auth.uid() instead of client-supplied actor_user_id.
DROP FUNCTION IF EXISTS transition_order_stage(uuid, text, text, int, uuid, text);
...
COMMENT ON FUNCTION transition_order_stage IS 'Atomic stage transition with optimistic lock + audit log. Actor derived from auth.uid() server-side. Returns {ok, code?, new_version?, new_sub_stage?, current_version?}.';
