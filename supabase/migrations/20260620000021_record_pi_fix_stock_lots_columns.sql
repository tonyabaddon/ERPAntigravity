-- supabase/migrations/20260620000021_record_pi_fix_stock_lots_columns.sql
-- Phase 2a hotfix: record_pi was writing wrong stock_lots columns.
-- The Phase 2 RPC INSERTed (sku, source_id, source_type, unit_cost, qty, ...)
-- but actual stock_lots table has columns (sku, po_id, unit_cost, qty_received,
-- qty_remaining, received_at, warehouse_id) — qty doesn't exist, and source_id
-- + source_type were added in 20260620000020.
--
-- This migration rewrites the record_pi RPC to use the correct column names:
--   qty → qty_received + qty_remaining (FIFO tracking, both start equal)
--   po_id stays NULL for Tagihan (FK is to purchase_orders, can't reuse for pi_id)
--   source_id = v_pi_id, source_type = 'TAGIHAN'
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(payload->'items') LOOP
    ...
    IF v_type = 'STOCK' THEN
      -- Corrected column names: qty_received + qty_remaining (FIFO), source_id + source_type
      INSERT INTO public.stock_lots (sku, source_id, source_type, unit_cost, qty_received, qty_remaining, received_at, warehouse_id)
      VALUES (v_sku, v_pi_id, 'TAGIHAN', v_unit_cost, v_qty, v_qty, now(), v_warehouse_id);
