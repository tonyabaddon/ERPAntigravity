-- supabase/migrations/20260620000022_record_pi_drop_stock_lots_warehouse_col.sql
-- Phase 2a hotfix v3: record_pi was still wrong — wrote stock_lots.warehouse_id which doesn't exist.
-- Warehouse routing belongs in stock_levels (the per-warehouse balance table), not stock_lots
-- (the FIFO cost-lot ledger). Drop warehouse_id from the stock_lots INSERT; keep stock_levels INSERT
-- which already correctly routes per-warehouse.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(payload->'items') LOOP
    ...
    IF v_type = 'STOCK' THEN
      -- stock_lots: FIFO cost-lot ledger. NO warehouse_id column (that's stock_levels).
      ...
      -- stock_levels: per-warehouse balance.
