...
-- FIXUP: Task 11 (slot 220) seeded admin_users.permissions with the wrong
-- key names (can_transfer_warehouse + can_receive_warehouse_transfer).
-- The existing project convention (PermissionSet type in src/types.ts and
-- pre-Wave permission ecosystem) uses `can_initiate_transfer` and
-- `can_receive_transfer` — same names already present in ALL_PERMISSIONS.
--
-- Actions:
--   1. Drop wrong keys from every admin_users.permissions row.
--   2. Seed correct keys (can_initiate_transfer + can_receive_transfer),
--      Owner=true, non-Owner=false — matching the pattern used by
--      20260613000004_backfill_can_manage_warehouses.sql.
--   3. Re-issue initiate_warehouse_transfer RPC with the correct key in
--      the receiver permission check.

BEGIN;

-- Step 1: Remove wrong keys
UPDATE public.admin_users
   SET permissions = permissions - 'can_transfer_warehouse' - 'can_receive_warehouse_transfer'
 WHERE permissions IS NOT NULL
   AND (permissions ? 'can_transfer_warehouse' OR permissions ? 'can_receive_warehouse_transfer');

-- Step 2a: Owner default true — can_initiate_transfer
UPDATE public.admin_users
 ...
 WHERE role = 'Owner'
   AND (permissions IS NULL OR NOT (permissions ? 'can_initiate_transfer'));

-- Step 2b: Owner default true — can_receive_transfer
UPDATE public.admin_users
   SET permissions = jsonb_set(
     COALESCE(permissions, '{}'::jsonb),
 ...
 WHERE role = 'Owner'
   AND (permissions IS NULL OR NOT (permissions ? 'can_receive_transfer'));

-- Step 2c: Non-Owner default false — can_initiate_transfer
UPDATE public.admin_users
   SET permissions = jsonb_set(
     COALESCE(permissions, '{}'::jsonb),
     '{can_initiate_transfer}',
...
-- Step 2d: Non-Owner default false — can_receive_transfer
UPDATE public.admin_users
   SET permissions = jsonb_set(
     COALESCE(permissions, '{}'::jsonb),
     '{can_receive_transfer}',
...
-- Step 3: Re-issue initiate_warehouse_transfer with correct receiver key
...
BEGIN
  ...
  -- Receiver check: must be tenant member AND have can_receive_transfer=true
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'TRANSFER_INVALID_RECEIVER: user % lacks can_receive_transfer or not in tenant', p_receiver_user_id;
  END IF;
