-- 20260626000006_approve_and_amend_owner_check_fix.sql
-- Phase 1B follow-up to PR #28: drop the over-strict `status='active'` check
-- from approve_and_amend_rakit_lock. Live admin_users.status uses Indonesian
-- 'Aktif' (and other variants), and the existing codebase Owner-check
-- convention (e.g., migration 20260613000002d) only filters by role='Owner'.
--
-- Applied to live Supabase during end-to-end smoke of PR #28 (Owner Biaya
-- Final integration). Committed here for source-of-truth parity.
...
BEGIN
  ...
  -- Match existing Owner-gate convention (migration 20260613000002d):
  -- filter on role only; status hardening is a separate cross-cutting fix
  -- (see memory: verify_owner_pin security gap).
  IF NOT EXISTS (
