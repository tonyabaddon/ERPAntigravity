...
--
-- Implementation note on label stamp placement:
--   Both RPCs have a validation loop and a separate output-build loop.
--   The validation loop mutates v_item locally but the results are discarded
--   (second loop re-reads from p_items / v_items_jsonb). Label stamp must
--   therefore live in the output-build pass, not the validation pass.
--   For record_kasir_sale: label is stamped in the second FOR loop
--   (v_item_out construction), using pricing_tier_used from the payload item.
--   For create_tempo_invoice: the validation loop is extended to also build
--   v_items_out with label stamped; that accumulator replaces v_items_jsonb
--   in the INSERT.
--   This mirrors the "stored as-is from payload" snapshot semantics documented
--   in slot 000325 at line 504-506. The defaulted 'eceran' tier (when modul ON
--   + SKU present + caller omits the key) does NOT inject into the snapshot —
--   matching the pre-existing contract.

-- ─── record_kasir_sale (widened for 4-tier) ──────────────────────────────────
...
-- ─── create_tempo_invoice (widened for 4-tier) ───────────────────────────────
