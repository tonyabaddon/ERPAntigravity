-- 20260724000001 — Phase 0c: HPP recognition in record_kasir_sale dual-write block
--
-- GAP CLOSED: Phase 0b dual-write posted only 2 GL lines per sale:
--   D <cash_coa>        p_total_amount   — cash received
--   K <pendapatan_coa>  p_total_amount   — revenue
--
-- This left gross margin inflated in P&L (COGS never hit the GL).
--
-- FIX: When v_kasir.hpp_total > 0, append 2 more lines to the JE:
--   D 5-1100 HPP Penjualan    v_kasir.hpp_total  — COGS recognised
--   K 1-1510 Persediaan       v_kasir.hpp_total  — inventory consumed
--
-- Balance check (SUM D = SUM K):
--   D = p_total_amount + hpp_total
--   K = p_total_amount + hpp_total   ✓
--
-- _post_journal_entry validates SUM(DEBIT)=SUM(CREDIT) across all lines
-- (not pairwise), so a 4-line JE passes.
--
-- When hpp_total = 0 (service-only sales, no FIFO lots), only 2 lines are
-- posted — identical to Phase 0b behaviour (back-compat).
--
-- Strategy: v_lines jsonb is declared in the inner DECLARE (inside the
-- EXCEPTION block) so any failure in line build or JE posting is still
-- caught by the existing WHEN OTHERS anomaly logger.
...
BEGIN
  ...
  DECLARE
    ...
    v_lines             jsonb;    -- Phase 0c: build lines conditionally
  BEGIN
    SELECT
    ...
    IF COALESCE(v_dual_write, false) THEN
      ...
      ELSE
        BEGIN
          ...
          -- Phase 0c: build JE lines. Always: D cash K pendapatan.
          -- Conditionally append D HPP K Persediaan when hpp_total > 0.
