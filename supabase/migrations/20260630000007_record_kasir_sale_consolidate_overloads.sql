-- 20260630000007 — record_kasir_sale: drop 3 overloads, restore canonical with allow_negative_stock
--
-- Production drifted into 3 overloads of record_kasir_sale:
--   1) 20p with p_marketplace_order_no              ← what the frontend tries to call
--   2) 21p with p_tokped_order_no + p_allow_negative_stock     ← stale migration 002
--   3) 21p with p_tokped_order_no + p_actor_user_id            ← parallel-session work
-- PostgREST refuses to disambiguate the wizard's 21-param call (#1's params +
-- p_allow_negative_stock) and 404s with PGRST202.
--
-- Resolution: drop all 3, create a single canonical signature that is
-- overload #1's body (the modern one — full marketplace channels, validate_sales_channel,
-- service-line aware) + p_allow_negative_stock appended with DEFAULT false.
--
-- p_actor_user_id is NOT used by any caller of record_kasir_sale; it's an
-- artifact of a parallel session attempting cross-RPC actor tracking. Dropped
-- to keep the surface area focused on what the wizard + legacy Kasir actually need.
--
-- p_allow_negative_stock is currently informational: the body already permits
-- underflow via deduct_stock_fifo RAISE WARNING. Hook stricter enforcement
-- when the requirement surfaces.
...
DROP FUNCTION IF EXISTS public.record_kasir_sale(
  date, text, jsonb, numeric, text, text, text, numeric, text,
  numeric, text, numeric, text, text, text, text, text, text, text, text, uuid
);
