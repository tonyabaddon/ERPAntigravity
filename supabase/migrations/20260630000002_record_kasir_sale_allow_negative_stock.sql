-- Migration 002: record_kasir_sale — add p_allow_negative_stock param
-- Phase: Catat Penjualan wizard
-- Purpose: Opt-in pre-order behavior. Default false = no behavior change.
--
-- The current body already permits silent underflow via deduct_stock_fifo
-- RAISE WARNING. The new param documents intent rather than gating behavior.
-- Future strict enforcement can hook the flag.
