...
func NewHandler(d *db.Client, m *engine.Machine, s *Sender, n *notification.Notifier, sc *scheduler.Scheduler, waNumberID, supabaseURL, supabaseServiceKey string, debounce debouncer) *Handler {
	return &Handler{
		db: d, machine: m, sender: s, notifier: n, scheduler: sc,
...
func (h *Handler) ProcessJoinedMessage(ctx context.Context, senderPhone, text string, originalTexts []string) {
	...
	if err != nil {
		slog.ErrorContext(ctx, "[HANDLER] GetOrCreateConversation error", slog.String("phone", senderPhone), slog.String("error", err.Error()))
		return
	}

	// Reset follow-up counter — customer has replied.
	if err := h.db.ResetFollowupCounter(conv.ID); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] ResetFollowupCounter error", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
	}
	...
	if parseErr != nil {
		slog.ErrorContext(ctx, "[HANDLER] Failed to parse tenant_id as UUID", slog.String("tenant_id", conv.TenantID), slog.String("error", parseErr.Error()))
		...
		if err != nil {
			slog.ErrorContext(ctx, "[HANDLER] GetOrCreateCustomer error", slog.String("phone", senderPhone), slog.String("error", err.Error()))
		} else {
			customerID = customer.ID
			if created {
				lead, err := h.db.CreateLead(customer.ID, conv.ID, senderPhone)
				if err != nil {
					slog.ErrorContext(ctx, "[HANDLER] CreateLead error", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
				} else {
					leadsID = lead.ID
				}
			}
	...
	if len(text) > 0 && text[0] >= '1' && text[0] <= '5' {
		if pendingOrder, hasPending, lookupErr := feedback.LookupPendingFeedback(ctx, h.db.DB, senderPhone); lookupErr != nil {
			slog.ErrorContext(ctx, "[HANDLER] LookupPendingFeedback error",
				slog.String("phone", senderPhone), slog.String("error", lookupErr.Error()))
		} else if hasPending {
			...
			if captureErr != nil {
				slog.ErrorContext(ctx, "[HANDLER] HandleFeedbackResponse error",
					slog.String("order_id", pendingOrder.OrderID), slog.String("error", captureErr.Error()))
			} else if captured {
				...
				if _, insertErr := h.db.InsertMessage(conv.ID, models.SenderAI, ack); insertErr != nil {
					slog.ErrorContext(ctx, "[HANDLER] feedback ack InsertMessage error", slog.String("error", insertErr.Error()))
				}
				if sendErr := h.sender.SendText(ctx, senderPhone, ack); sendErr != nil {
					slog.ErrorContext(ctx, "[HANDLER] feedback ack send error", slog.String("error", sendErr.Error()))
				}
				return
			}
		}
	...
	if conv.StateLockedUntil != nil && conv.StateLockedUntil.Before(time.Now()) {
		if err := h.db.AutoResumeConv(ctx, conv.ID); err == nil {
			...
			slog.ErrorContext(ctx, "[HANDLER] AutoResumeConv failed", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
		}
	}
	...
	if conv.State == models.StateBooked || conv.State == models.StateTimeoutReminder {
		...
		if _, err := h.db.InsertMessage(conv.ID, models.SenderAI, reply); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] BOOKED InsertMessage error", slog.String("error", err.Error()))
		}
		if err := h.sender.SendText(ctx, senderPhone, reply); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] BOOKED holding reply send error", slog.String("error", err.Error()))
		}
		return
	}
	...
	if conv.State == models.StateCompleted || conv.State == models.StateCancelled {
		if err := h.db.UpdateConversationState(conv.ID, models.StateGreeting); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] Reset conv state error", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
			return
		}
	...
	for _, original := range texts {
		if _, err := h.db.InsertMessage(conv.ID, models.SenderCustomer, original); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] InsertMessage error", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
	...
	result := engine.RetryProcess(ctx, h.machine, conv, text, history, stockContext, 3, func() {
		...
		if sendErr := h.sender.SendText(ctx, senderPhone, holdingMsg); sendErr != nil {
			slog.ErrorContext(ctx, "[HANDLER] holding message send error", slog.String("error", sendErr.Error()))
		}
	})

	if result.LLMError != nil {
		slog.ErrorContext(ctx, "[HANDLER] LLM failed after all retries", slog.String("phone", senderPhone), slog.String("error", result.LLMError.Error()))
		...
		if dbErr := h.db.UpdateConversationState(conv.ID, models.StateEscalatedAdmin); dbErr != nil {
			slog.ErrorContext(ctx, "[HANDLER] UpdateConversationState (escalation) error", slog.String("error", dbErr.Error()))
		}
		recipients, recErr := h.db.GetActiveRecipients()
		if recErr != nil {
			slog.ErrorContext(ctx, "[HANDLER] GetActiveRecipients error during escalation", slog.String("error", recErr.Error()))
			return
		}
		...
		for _, r := range recipients {
			if notifErr := h.sender.SendText(ctx, r.WANumber, notif); notifErr != nil {
				slog.ErrorContext(ctx, "[HANDLER] escalation notify error", slog.String("wa_number", r.WANumber), slog.String("error", notifErr.Error()))
			}
		}
		return
	}
	...
	if conv.StateLockedUntil != nil && conv.StateLockedUntil.After(time.Now()) {
		...
		if result.NewData != nil {
			if err := h.db.UpdateCollectedData(conv.ID, *result.NewData, result.ClarificationRound); err != nil {
				slog.ErrorContext(ctx, "[HANDLER] UpdateCollectedData error", slog.String("error", err.Error()))
			}
		}
		if result.Language != conv.Language {
			if err := h.db.UpdateLanguage(conv.ID, result.Language); err != nil {
				slog.ErrorContext(ctx, "[HANDLER] UpdateLanguage error", slog.String("error", err.Error()))
			}
		}
		if result.NextState != conv.State {
			if err := h.db.UpdateConversationState(conv.ID, result.NextState); err != nil {
				slog.ErrorContext(ctx, "[HANDLER] UpdateConversationState error", slog.String("error", err.Error()))
			}
		}
	}
	...
	if result.Reply != "" {
		...
		if err := h.sender.SendText(ctx, senderPhone, result.Reply); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] SendText error", slog.String("error", err.Error()))
		}
	}
}
...
func (h *Handler) handleBooking(ctx context.Context, conv *models.Conversation, leadsID, customerID string, deliveryType models.DeliveryType) {
	...
	if err != nil {
		slog.ErrorContext(ctx, "[HANDLER] CreateOrder error", slog.String("error", err.Error()))
		return
	}
...
func (h *Handler) handleWiringEscalation(ctx context.Context, senderPhone, text string) {
	...
	if parseErr != nil {
		...
		customer, err := h.db.GetOrCreateCustomer(tenantUUID, senderPhone)
		if err != nil {
			slog.ErrorContext(ctx, "[HANDLER] handleWiringEscalation: GetOrCreateCustomer error", slog.String("phone", senderPhone), slog.String("error", err.Error()))
		} else if created {
			if _, err := h.db.CreateLead(customer.ID, conv.ID, senderPhone); err != nil {
				slog.ErrorContext(ctx, "[HANDLER] handleWiringEscalation: CreateLead error", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
			}
		}
	}
	...
	if err := h.sender.SendText(ctx, senderPhone, reply); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] handleWiringEscalation: SendText error", slog.String("error", err.Error()))
	}
}
...
func (h *Handler) handleAdminEscalation(ctx context.Context, senderPhone, text string) {
	conv, created, err := h.db.GetOrCreateConversation(senderPhone, h.waNumberID)
	if err != nil {
		slog.ErrorContext(ctx, "[HANDLER] handleAdminEscalation: GetOrCreateConversation error", slog.String("phone", senderPhone), slog.String("error", err.Error()))
		return
	}
	...
	if parseErr != nil {
		...
		customer, err := h.db.GetOrCreateCustomer(tenantUUID, senderPhone)
		if err != nil {
			slog.ErrorContext(ctx, "[HANDLER] handleAdminEscalation: GetOrCreateCustomer error", slog.String("phone", senderPhone), slog.String("error", err.Error()))
		} else if created {
			if _, err := h.db.CreateLead(customer.ID, conv.ID, senderPhone); err != nil {
				slog.ErrorContext(ctx, "[HANDLER] handleAdminEscalation: CreateLead error", slog.String("conv_id", conv.ID), slog.String("error", err.Error()))
			}
		}
	}
	...
	h.db.InsertMessage(conv.ID, models.SenderAI, reply)
	if err := h.sender.SendText(ctx, senderPhone, reply); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] handleAdminEscalation: SendText error", slog.String("error", err.Error()))
	}
}
...
func (h *Handler) handleMediaMessage(evt *events.Message) {
	...
	if img != nil {
		...
		if dlErr != nil {
			slog.Error("[HANDLER] DownloadMedia error", slog.String("order_id", order.ID), slog.String("error", dlErr.Error()))
		} else {
			...
			if upErr != nil {
				slog.Error("[HANDLER] UploadPaymentProof error", slog.String("order_id", order.ID), slog.String("error", upErr.Error()))
			} else {
				proofURL = url
			}
		}
		...
		if dlErr != nil {
			slog.Error("[HANDLER] DownloadDocument error", slog.String("order_id", order.ID), slog.String("error", dlErr.Error()))
		} else {
			url, upErr := storage.UploadPaymentProof(context.Background(), h.supabaseURL, h.supabaseServiceKey, tenantID, order.ID, data, contentType)
			if upErr != nil {
				slog.Error("[HANDLER] UploadPaymentProof error", slog.String("order_id", order.ID), slog.String("error", upErr.Error()))
	...
	case models.OrderStatusWaitingDP, models.OrderStatusDPUploaded:
		if err := h.db.UpdateDPProof(order.ID, proofURL); err != nil {
			slog.Error("[HANDLER] UpdateDPProof error", slog.String("order_id", order.ID), slog.String("error", err.Error()))
		}
	default: // WAITING_PAYMENT, PAYMENT_UPLOADED, DP_VERIFIED
		if err := h.db.UpdatePaymentProof(order.ID, proofURL); err != nil {
			slog.Error("[HANDLER] UpdatePaymentProof error", slog.String("order_id", order.ID), slog.String("error", err.Error()))
		}
	}
	...
	if err := h.sender.SendText(context.Background(), senderPhone, ack); err != nil {
		slog.Error("[HANDLER] Payment ack send error", slog.String("error", err.Error()))
	}

	recipients, err := h.db.GetActiveRecipients()
	if err != nil {
		slog.Error("[HANDLER] GetActiveRecipients error", slog.String("error", err.Error()))
		return
	}
	...
	for _, r := range recipients {
		if err := h.sender.SendText(context.Background(), r.WANumber, notif); err != nil {
			slog.Error("[HANDLER] Recipient notify error", slog.String("wa_number", r.WANumber), slog.String("error", err.Error()))
		}
	}
}
...
func (h *Handler) HandleApprovedOrder(ctx context.Context, orderID, conversationID string, shippingFee float64) {
	...
	if err != nil || order == nil {
		slog.ErrorContext(ctx, "[HANDLER] GetOrderByConversation error", slog.String("conversation_id", conversationID), slog.String("error", err.Error()))
		return
	}
	...
	if err := h.db.UpdateOrderTotal(orderID, total); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] UpdateOrderTotal error", slog.String("error", err.Error()))
	}

	bank, err := h.db.GetActiveBankConfig()
	if err != nil {
		slog.WarnContext(ctx, "[HANDLER] GetActiveBankConfig error (using fallback)", slog.String("error", err.Error()))
	}
	...
	if order.PaymentType == "DP" {
		...
		if buildErr != nil {
			slog.ErrorContext(ctx, "[HANDLER] OrderApproved template build error", slog.String("error", buildErr.Error()))
		...
		// Route through NotifyCustomer: quota enforcement + atomic audit trail.
		// Log-and-continue on error so order status still advances (quota block
		// must not strand the order in an unconfirmed state).
		if err := h.notifier.NotifyCustomer(ctx, order.TenantID, conversationID, order.CustomerPhone, "id", dpMsg); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] OrderApproved NotifyCustomer error", slog.String("error", err.Error()))
		}
		...
		if err := h.sender.SendText(ctx, order.CustomerPhone, invoice); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] Invoice send error", slog.String("error", err.Error()))
		}
	...
	recipients, err := h.db.GetActiveRecipients()
	if err != nil {
		slog.ErrorContext(ctx, "[HANDLER] GetActiveRecipients error", slog.String("error", err.Error()))
	} else {
		orderRef := order.GJPOrderID
		if orderRef == "" {
			orderRef = orderID
		...
		for _, r := range recipients {
			if err := h.sender.SendText(ctx, r.WANumber, notif); err != nil {
				slog.ErrorContext(ctx, "[HANDLER] Recipient notify error", slog.String("wa_number", r.WANumber), slog.String("error", err.Error()))
			}
		}
	}
...
func (h *Handler) HandlePaymentVerified(ctx context.Context, orderID, conversationID string) {
	...
	if err != nil || order == nil {
		slog.ErrorContext(ctx, "[HANDLER] HandlePaymentVerified: GetOrderByIDWithPayment error", slog.String("order_id", orderID), slog.String("error", err.Error()))
		return
	}
	...
	if buildErr != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandlePaymentVerified: template build error", slog.String("error", buildErr.Error()))
	...
	// Route through NotifyCustomer: quota enforcement + atomic customer-message audit.
	// Log-and-continue so downstream stock deduction and state update still proceed.
	if err := h.notifier.NotifyCustomer(ctx, order.TenantID, conversationID, order.CustomerPhone, "id", msg); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandlePaymentVerified: NotifyCustomer error", slog.String("error", err.Error()))
	}
	...
	if order.LeadsID != "" {
		if err := h.db.UpdateLeadStatus(order.LeadsID, models.LeadStatusOrdered); err != nil {
			slog.ErrorContext(ctx, "[HANDLER] UpdateLeadStatus error", slog.String("lead_id", order.LeadsID), slog.String("error", err.Error()))
		}
	}
	...
	for _, item := range order.Items {
		...
		if err != nil {
			slog.ErrorContext(ctx, "[HANDLER] DeductStockAndGetHPP error", slog.String("sku", item.SKU), slog.Int("qty", item.Qty), slog.String("error", err.Error()))
			continue
		}
		totalHpp += cost
	}
	if err := h.db.UpdateOrderHpp(orderID, totalHpp); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] UpdateOrderHpp error", slog.String("order_id", orderID), slog.String("error", err.Error()))
	}
}
...
func (h *Handler) HandlePaymentRejected(ctx context.Context, orderID, conversationID string) {
	order, err := h.db.GetOrderByConversation(conversationID)
	if err != nil || order == nil {
		slog.ErrorContext(ctx, "[HANDLER] HandlePaymentRejected: GetOrderByConversation error", slog.String("conversation_id", conversationID), slog.String("error", err.Error()))
		return
	}
	...
	if buildErr != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandlePaymentRejected: template build error", slog.String("error", buildErr.Error()))
	...
	// Route through NotifyCustomer: quota enforcement + atomic customer-message audit.
	// Log-and-continue so RejectPayment still runs even on quota block.
	if err := h.notifier.NotifyCustomer(ctx, order.TenantID, conversationID, order.CustomerPhone, "id", msg); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandlePaymentRejected: NotifyCustomer error", slog.String("error", err.Error()))
	}
	...
	if err := h.db.RejectPayment(orderID); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] RejectPayment error", slog.String("order_id", orderID), slog.String("error", err.Error()))
	}
}
...
func (h *Handler) HandleDPVerified(ctx context.Context, orderID, conversationID string) {
	order, err := h.db.GetOrderByConversation(conversationID)
	if err != nil || order == nil {
		slog.ErrorContext(ctx, "[HANDLER] HandleDPVerified: GetOrderByConversation error", slog.String("conversation_id", conversationID), slog.String("error", err.Error()))
		return
	}
	...
	if buildErr != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandleDPVerified: template build error", slog.String("error", buildErr.Error()))
	...
	// Route through NotifyCustomer: quota enforcement + atomic customer-message audit.
	// Log-and-continue so the system InsertMessage still records the event on error.
	if err := h.notifier.NotifyCustomer(ctx, order.TenantID, conversationID, order.CustomerPhone, "id", msg); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandleDPVerified: NotifyCustomer error", slog.String("error", err.Error()))
	}
...
func (h *Handler) HandleDPProofRejected(ctx context.Context, orderID, conversationID, reason string) {
	order, err := h.db.GetOrderByConversation(conversationID)
	if err != nil || order == nil {
		slog.ErrorContext(ctx, "[HANDLER] HandleDPProofRejected: GetOrderByConversation error", slog.String("conversation_id", conversationID), slog.String("error", err.Error()))
		return
	}
	...
	if err := h.sender.SendText(ctx, order.CustomerPhone, msg); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] HandleDPProofRejected: SendText error", slog.String("error", err.Error()))
	}
	...
	if err := h.db.ResetDPToWaiting(orderID); err != nil {
		slog.ErrorContext(ctx, "[HANDLER] ResetDPToWaiting error", slog.String("order_id", orderID), slog.String("error", err.Error()))
	}
}
