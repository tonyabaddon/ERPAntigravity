...
func (m *Machine) Process(ctx context.Context, conv *models.Conversation, incomingText string, history []models.Message, stockContext string) (*ProcessResult, error) {
	...
	if err != nil {
		slog.ErrorContext(ctx, "[ENGINE] LLM error", slog.String("state", string(conv.State)), slog.String("error", err.Error()))
	...
	case models.StateGreeting:
		...
		if err != nil {
			slog.ErrorContext(ctx, "[ENGINE] Parse greeting error", slog.String("error", err.Error()), slog.String("raw", rawJSON))
			result.Reply = FallbackReply(conv.Language)
			return result, nil
		}
	...
	case models.StateCollecting:
		...
		if err != nil {
			slog.ErrorContext(ctx, "[ENGINE] Parse collecting error", slog.String("error", err.Error()), slog.String("raw", rawJSON))
			result.Reply = FallbackReply(conv.Language)
			return result, nil
		}
		result.Reply = resp.Reply
	...
	case models.StateClarifying:
		...
		if err != nil {
			slog.ErrorContext(ctx, "[ENGINE] Parse clarifying error", slog.String("error", err.Error()), slog.String("raw", rawJSON))
			result.Reply = FallbackReply(conv.Language)
			return result, nil
		}
		result.Reply = resp.Reply
	...
	case models.StateStockCheck:
		...
		if err != nil {
			slog.ErrorContext(ctx, "[ENGINE] Parse stock_check error", slog.String("error", err.Error()), slog.String("raw", rawJSON))
			result.Reply = FallbackReply(conv.Language)
			return result, nil
		}
		result.Reply = resp.Reply
	...
	case models.StateConfirming:
		...
		if err != nil {
			slog.ErrorContext(ctx, "[ENGINE] Parse confirming error", slog.String("error", err.Error()), slog.String("raw", rawJSON))
			result.Reply = FallbackReply(conv.Language)
			return result, nil
		}
		result.Reply = resp.Reply
	...
	case models.StateDelivery:
		...
		if err != nil {
			slog.ErrorContext(ctx, "[ENGINE] Parse delivery error", slog.String("error", err.Error()), slog.String("raw", rawJSON))
			result.Reply = FallbackReply(conv.Language)
			return result, nil
		}
		result.Reply = resp.Reply
